    ...
    public function showRatingScreen($matchId)
    {
        try {
            ...
            return response()->json([
                'match' => $match,
                'team_players' => $userTeam->players->map(function ($player) {
                    return [
    ...
    private function updateUserStats($matchId)
    {
        try {
            ...
            foreach ($players as $player) {
                ...
                $avgRating = $userRatings->isEmpty() ? 0 : $userRatings->avg('rating');
                $avgAttitude = $userRatings->isEmpty() ? 0 : $userRatings->avg('attitude_rating');
                $avgParticipation = $userRatings->isEmpty() ? 0 : $userRatings->avg('participation_rating');
                ...
                $stats->update([
                    ...
                    'average_attitude' => round($avgAttitude, 2),
